...
type TodayPageProps={tasks:Task[];health:Health;profile:Profile;setHealth:Dispatch<SetStateAction<Health>>;onToggle:(id:number)=>void;onAdd:()=>void;onOpenTasks:()=>void;onOpenCapacity:()=>void;onFocus:()=>void}
...
export function TodayPage({tasks,health,profile,setHealth,onToggle,onAdd,onOpenTasks,onOpenCapacity,onFocus}:TodayPageProps){
  ...
  const candidates=today.length?today:open
  const top=candidates.filter(task=>task.quadrant==='Q1'||task.quadrant==='Q2').slice(0,3)
  ...
  return <>
    ...
    <section className="card now-card"><div><span className="kicker">現在</span><h2>{top[0]?.title||'先選一件最重要的事'}</h2><p>{top[0]?`預計 ${top[0].estimate} 分鐘・${top[0].category||'一般'}`:'不用把全部做完，先讓一件事往前。'}</p></div><button className="primary" onClick={onFocus}><Play size={17}/>開始專注</button></section>
