...
export function CalendarPage({tasks,setTasks,events,setEvents,onUndo,projects=[],roles=[],health={water:0,sleep:7,energy:3,stress:3,mood:3,exercise:0}}:{tasks:Task[];setTasks:Dispatch<SetStateAction<Task[]>>;events:CalendarEvent[];setEvents:Dispatch<SetStateAction<CalendarEvent[]>>;onUndo:(item:CalendarEvent)=>void;projects?:Project[];roles?:string[];health?:Health}){
  ...
  const eventsOn=(date:Date)=>events.filter(event=>event.date===isoDate(date)||event.recurrence==='每天'||(event.recurrence==='每週'&&new Date(`${event.date}T00:00:00`).getDay()===date.getDay()))
  ...
  const eventChip=(event:CalendarEvent)=><span className={`cal-event category-${event.category}`} key={event.id}><strong>{event.start}</strong> {event.title}{event.recurrence!=='無'&&<em>{event.recurrence}</em>}<button aria-label={`刪除 ${event.title}`} onClick={()=>removeEvent(event)}>×</button></span>
